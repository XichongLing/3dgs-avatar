...
if __name__ == '__main__':
    # TODO: manually adjust the dir and model type
    root_dir = '../project_lx/datasets/4Dress/00185'
    outfit = 'Inner'
    model_type = 'smpl'
    gender = 'female'
    ...
    if model_type == 'smplx':
        ...
        all_takes = glob.glob(os.path.join(root_dir, outfit, '*'))
        ...
        bm_path = 'body_models/smpl/' + gender + '/model.pkl'
        # Extract minimal shape for smpl
        body_model = BodyModel(bm_path=bm_path, num_betas=10, batch_size=1).to(device)
        all_takes = glob.glob(os.path.join(root_dir, outfit, '*'))
        for take_dir in all_takes:
            print("Processing {}".format(take_dir))
            if not os.path.exists(os.path.join(take_dir, 'SMPL_processed')):
                os.makedirs(os.path.join(take_dir, 'SMPL_processed'))
            smpl_files = glob.glob(os.path.join(take_dir, 'SMPL', '*.pkl'))
            pointcloud_files = glob.glob(os.path.join(take_dir, 'SMPL', '*.ply'))
            assert len(smpl_files) == len(pointcloud_files)
            for smpl_file, pointcloud_file in tqdm(zip(smpl_files, pointcloud_files), total=len(smpl_files)):
                params = np.load(smpl_file, allow_pickle=True)
                root_orient = Rotation.from_rotvec(np.array(params['global_orient']).reshape([-1])).as_matrix()
                ...
                betas = np.array(params['betas'], dtype=np.float32).reshape(1, -1)
                pose_body = np.array(params['body_pose'], dtype=np.float32).reshape(1, -1)
                pose_hand = pose_body[:, 63:]
                pose_body = pose_body[:, :63]
                poses = np.concatenate(
                    [np.zeros((1, 3)), pose_body, pose_hand], axis=-1).astype(np.float32)
                poses_torch = torch.from_numpy(poses).to(device)
                pose_body_torch = torch.from_numpy(pose_body).to(device)
                ...
                # Get bone transforms
                body = body_model(root_orient=new_root_orient_torch, pose_body=pose_body_torch,
                                  pose_hand=pose_hand_torch, betas=betas_torch, trans=new_trans_torch)
                body_model_em = load_model(gender=gender, model_type='smpl').to(device)
                verts = body_model_em(poses=poses_torch, shapes=betas_torch,
                                      Rh=new_root_orient_torch, Th=new_trans_torch,
                                      return_verts=True)[0].detach().cpu().numpy()
                vertices = body.v.detach().cpu().numpy()[0]
                new_trans = new_trans + (verts - vertices).mean(0, keepdims=True)
                ...
                Jtr_posed = body.Jtr.detach().cpu().numpy()
                file_name = smpl_file.split('/')[-1]
                out_filename = os.path.join(take_dir, 'SMPL_processed', file_name)
                np.savez(out_filename,
                         minimal_shape=minimal_shape,
                         betas=betas,
                         Jtr_posed=Jtr_posed[0],
                         ...
                         transl=new_trans[0],
                         global_orient=new_root_orient[0],
                         body_pose=np.concatenate([pose_body[0], pose_hand[0]], axis=-1))